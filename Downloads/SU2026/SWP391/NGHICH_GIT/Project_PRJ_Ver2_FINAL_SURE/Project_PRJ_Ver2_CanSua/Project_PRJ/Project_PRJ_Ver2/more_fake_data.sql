-- Script thêm data fake cho các tháng còn thiếu
-- Giả sử hôm nay là tháng 12/2024, ta thêm data cho các tháng trong năm 2024

-- Xóa data cũ nếu cần (chỉ chạy nếu muốn reset)
-- DELETE FROM OrderDetails WHERE orderId IN (SELECT orderId FROM Orders WHERE YEAR(createdAt) = 2024);
-- DELETE FROM Orders WHERE YEAR(createdAt) = 2024;

-- Thêm orders fake cho các tháng chưa có data
...
PRINT '📈 Tong doanh thu nam 2024: ~ 650M VND';
PRINT '📦 Tong orders: 60 orders (55 COMPLETED, 5 PENDING, 5 CANCEL)';
PRINT '👥 Users: Dùng userId có sẵn (1=admin, 2=user1)';
PRINT '';
PRINT '🎉 Biêu do se rat dep va chuyen nghiep!';
PRINT '    - Tang truong deu tu thang 1 -> 12';
PRINT '    - Cac dot cao: Thang 7-8 (mua he), Thang 11-12 (le tet)';
PRINT '    - Phan bo status tu nhien: 92% COMPLETED, 8% PENDING, 8% CANCEL';
PRINT '';
PRINT '🚀 Hay restart web application va test dashboard!';
